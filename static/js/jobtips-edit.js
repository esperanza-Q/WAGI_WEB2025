...
const input = document.getElementById("jobtips-edit-upload-input");
const preview = document.getElementById("jobtips-edit-upload-preview");
const deletedFilesInput = document.getElementById("jobtips-deleted-files"); 
...
function renderPreview() {
    ...
    existingFiles.forEach((file, index) => {
        ...
        item.classList.add("jobtips-edit-preview-item");
        if (file.type === "image") {
            const img = document.createElement("img");
            img.classList.add("jobtips-edit-preview-img");
            img.src = file.url;
            item.appendChild(img);
        } else {
            const fileBox = document.createElement("div");
            fileBox.classList.add("jobtips-edit-preview-file");
            fileBox.textContent = file.name;
            item.appendChild(fileBox);
        }
        // 삭제 버튼
        const removeBtn = document.createElement("div");
        removeBtn.classList.add("jobtips-edit-preview-remove");
...
function renderTags() {
    ...
    tags.forEach((tag, index) => {
        ...
        tagItem.classList.add("jobtips-edit-tag-item");
        tagItem.innerHTML = `
            ${tag}
            <span class="jobtips-edit-tag-remove" data-index="${index}">×</span>
        `;
        tagList.appendChild(tagItem);
    });
    document.querySelectorAll(".jobtips-edit-tag-remove").forEach(btn => {
